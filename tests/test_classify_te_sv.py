...
class ClassifyTeSvTests(unittest.TestCase):
    ...
    def test_write_deduplicated_query_fasta_uses_sequence_md5(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir)
            query = base / "panpop_alleles.fa"
            dedup = base / "panpop_alleles.dedup.fa"
            query.write_text(
                ">q1\nAACCGGTT\n"
                ">q2\nAACCGGTT\n"
                ">q3\nTTTTGGGG\n",
            ...
            count = write_deduplicated_query_fasta(query, dedup)
    ...
    def test_integrated_workflow_generates_fastas_runs_blast_and_writes_tip_vcf(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            dedup_query_exists = (workdir / "panpop_alleles.dedup.fa").is_file()
        ...
        self.assertEqual(commands[1][commands[1].index("-dust") + 1], "no")
        self.assertIn(sequence_md5(alt), index_text)
        self.assertEqual(record_count, 1)
        self.assertEqual(report_count, 1)
        self.assertTrue(allele_fasta_exists)
        self.assertTrue(dedup_query_exists)
    ...
    def test_integrated_workflow_splits_deduplicated_query_for_parallel_blast(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir)
            panpop = base / "panpop.vcf"
            sv_fasta = base / "sv.fa"
            ...
            panpop.write_text(
                "##fileformat=VCFv4.2\n"
                "#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\tA\n"
                "Chr1\t10\tins1\tA\t%s,%s\t.\tPASS\t.\tGT\t1/2\n" % (alt1, alt2),
                encoding="utf-8",
            )
            sv_fasta.write_text(">%s\n%s\n" % (te_seq_id, alt1), encoding="utf-8")
            te_tsv.write_text(
                "seq_id\tmd5\tchrom\tstart\tend\tstrand\tsource\ttype\tfamily\tclass\tattributes\n"
                "%s\tmd5te\tChr1\t1\t70\t+\tEDTA\trepeat_region\tGypsy\tLTR\tID=x\n" % te_seq_id,
                encoding="utf-8",
            ...
            def runner(command: Sequence[str], cwd: Path | None) -> subprocess.CompletedProcess[str]:
                ...
                if command[0] == "blastn":
                    query = Path(command[command.index("-query") + 1])
                    out = Path(command[command.index("-out") + 1])
                    first_record = next(iter_fasta(query))
                    subject = "%s::te:1-70:fragmentmd5" % te_seq_id
                    out.write_text(
                        "%s\t%s\t90\t70\t0\t0\t1\t70\t1\t70\t1e-20\t200\n" % (first_record.name, subject),
                        encoding="utf-8",
                    )
                return subprocess.CompletedProcess(command, 0, "", "")
            ...
            blast_lines = (workdir / "panpop_allele_vs_te.tsv").read_text(encoding="utf-8").strip().splitlines()

        blast_commands = [command for command in commands if command[0] == "blastn"]
        self.assertEqual(len(blast_commands), 2)
        self.assertTrue(all("blast_query_parts" in command[command.index("-query") + 1] for command in blast_commands))
        self.assertTrue(all(command[command.index("-num_threads") + 1] == "2" for command in blast_commands))
